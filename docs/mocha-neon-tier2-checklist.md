...
- [x] **Spotify (Spicetify)** — Catppuccin theme installed + `~/.config/spicetify/Themes/catppuccin/color.ini` Mocha section bumped (tone-down fix applied: highlight/button use surface, mauve only on `button-active`/`tab-active`/`selected-row`). Applied by `run_onchange_after_41-spicetify-apply`. Screenshot: `mocha-neon-tier2/spotify.png`.
- [x] **Firefox Developer Edition** — `dot_config/firefox/userChrome.css` synced into every Firefox profile by `run_onchange_after_39-firefox-userchrome-sync` (both default + dev-edition-default). Requires one-time toggle of `toolkit.legacyUserProfileCustomizations.stylesheets = true` in `about:config` per profile. Screenshot: `mocha-neon-tier2/firefox.png`.
