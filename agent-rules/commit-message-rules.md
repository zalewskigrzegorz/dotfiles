...
5. The `g-commit` skill (`agent-skills/g-commit/SKILL.md`) is the source of truth for format: conventional commits, lowercase imperative subject, one trailing gitmoji, scope per repo's `commitlint.config.js` (or Redocly defaults).
