...
Work repositories (Redocly and any other org-owned repo) do not accept AI scratch artifacts. Committing brainstorm notes, plans, or design docs from `superpowers` into them pollutes diffs, PRs and history. Personal repositories are fine to keep their own conventions.
...
`<repo-basename>` = the basename of the repo root (e.g. `redocly-cms`, `reunite`, `realm`). Create the directory if missing — never refuse the skill, just relocate.
