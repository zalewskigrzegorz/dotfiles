...
`<repo-basename>` = the basename of the repo root the user is currently working in (e.g. `dotfiles`, `REDACTED_ORG-cms`, `reunite`, `realm`). If the user is not inside a git repo, use `scratch/` or another descriptive folder. Create the directory if missing — never refuse the skill, just relocate.
