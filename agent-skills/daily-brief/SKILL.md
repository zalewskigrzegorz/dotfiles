...
- **1 (Monday) or 3 (Wednesday)** → **Mon/Wed mode**: include section 8 (Cyberpunks pre-sync rundown) before the closing. Sync is at 12:00.
...
**Exception — keep the event** even from those calendars if Greg is in the `Attendees:` list AND it's not a public holiday (he was explicitly invited to a shared-calendar event = it concerns him). Greg's email anchors: `grzegorz.zalewski@redocly.com`, `maksim009@gmail.com`, `zalewski.grzegorz@icloud.com`.
...
For surviving entries: if attendees include Cyberpunks roster, mention it; if external attendee (non-redocly.com domain), mention the meeting + the attendee's company.
...
### 8. Cyberpunks pre-sync — ONLY on Mon/Wed
...
- Support / Slack threads where that person pinged Greg or where Greg owes a reply (`pull-requests-review`, support tickets, DMs).
- Outstanding action items mutually owed (from Hindsight `wip-context` / `tomorrow` tags matched to that person).
- Recent Cyberpunks-scope incidents touching the person's area (for Adam: SSO/RBAC; for Roman: API gateway / Reunite; etc.).
