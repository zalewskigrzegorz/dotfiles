...
name: bsk
description: |
  The WATCHED browser — drives Greg's real, logged-in Comet profile in a visible
  Agent Window. Use when he says "bsk", "basek", "odpal bsk", "w Comecie",
  "chcę to widzieć", "zobacz ze mną", "poszukajmy razem", "wejdź na moje
  konto" — or when the task needs his login/session, is behind SSO, gets
  bot-blocked, needs a captcha or 2FA, or is a long multi-step flow he should
  watch. For a quick unattended shot at a public page use `agent-browser`
  instead — it's cheaper in tokens and opens no window.
  Requires the bsk CLI installed and the BrowserSkill extension loaded in Comet.
---
